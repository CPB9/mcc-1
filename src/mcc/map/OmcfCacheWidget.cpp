...
void OmcfCacheWidget::getFiles(int baseLen, const char* format, QDir* startDir, OrderedTilePosCache<QString>* tilePaths)
{
    ...
    for (const QString& dir : dirs) {
        startDir->cd(dir);
        getFiles(baseLen, format, startDir, tilePaths);
        startDir->cdUp();
    }
}

void OmcfCacheWidget::createCache(const QString& dirPath, const QString& outputPath, const QString& name,
                                  const QString& description, const QByteArray& format, const mccgeo::MercatorProjection& proj)
{
    OrderedTilePosCache<QString> tiles;
    QDir inputDir(dirPath);

    getFiles(inputDir.absolutePath().length() + 1, format.constData(), &inputDir, &tiles);
    int count = (int)tiles.count();
    ...
    header.writeUint32Le(OmcfCache::headerMagic);
    ...
    auto writer = [this, &header, &inputDir, &output, &currentOffset, &count](const TilePosition& pos,
        ...
        header.writeUint16Le(pos.zoomLevel + 1);  //backwards compatible zoom
    ...
    tiles.map(writer);
    uint32_t crc = OmcfCache::crc32(header.start(), header.sizeUsed());
    header.writeUint32Le(crc);
    BMCL_ASSERT(header.writableSize() == 0);
    output.seek(0);
    ...
    : QWidget(parent)
{
    ...
    connect(createButton, &QPushButton::clicked, this,
            [this, openDirEdit, saveFileEdit, imageFormatBox, projectionBox, nameEdit, descriptionEdit]() {
                ...
                mccgeo::MercatorProjection proj;
                if (projectionBox->currentIndex() == 0) {
                    proj = mccgeo::MercatorProjection(mccgeo::MercatorProjection::SphericalMercator);
                } else {
                    proj = mccgeo::MercatorProjection(mccgeo::MercatorProjection::EllipticalMercator);
                }
...
OmcfCacheWidget::~OmcfCacheWidget()
{
}
}
