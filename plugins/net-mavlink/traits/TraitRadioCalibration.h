...
public:
    ...
    TraitRadioCalibration(caf::actor_config& cfg, const caf::actor& core, const caf::actor& broker, const mccmsg::ProtocolId& id, const std::string& name, const MavlinkSettings& settings);
    ...
    void processChannelsMessage(mavlink_rc_channels_t& channels);
...
private:
    void handleChannelsMessage(const mavlink_rc_channels_t& channels);
    ...
    void writeParameter(const QString& id, float value);
    void writeParameterInt(const QString& id, int value);
    ...
    bool _started;
    size_t _cancelCalibrationCounter;
};
}
